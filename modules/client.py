...
def apply(table):
    ...
    if(tempbuff):
        messagebox.showinfo(
            'Oups !', 'Il semble que vous ayez déjà postulé à cette offre')
    else:
        ...
        mycon.commit()
        mycon.close()
        messagebox.showinfo('Merci', 'Votre candidature a bien été reçue')
...
def delet(table):
    ...
    mycon.commit()
    mycon.close()
    messagebox.showinfo('Merci', 'Votre candidature a bien été supprimée')
    myapp()
...
def available():
    ...
    search_l = Label(rt, text="Trier : ", font=(
        'normal', 18), bg="#ffffff")
...
def myapp():
    ...
    search_l = Label(rt, text="Trier : ", font=('normal', 18), bg="#ffffff")
    search_l.grid(row=0, column=0, padx=10, pady=10)
    global search_d
    search_d = ttk.Combobox(rt, width=12, font=(
        'normal', 18), state='readonly')
    ...
    search_d.current(0)
    search_d.grid(row=0, column=2, padx=0, pady=10)
    search = Button(rt, text="genre", font=('normal', 12, 'bold'), bg="#00b9ed",
                    fg="#ffffff", command=lambda: sort_myapplications(table))
    search.grid(row=0, column=3, padx=10, pady=10, ipadx=15)

    dlt = Button(rt, text="Supprimer", font=('normal', 12, 'bold'),
...
def cli(root, email1):
    ...
    bn = Button(root, text="Déconnexion", font=('normal', 20),
